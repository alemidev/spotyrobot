	...
	try:
		...
		if q.startswith("spotify:") or q.startswith("http://open.spotify.com"):
			...
			text = join_artists(res["tracks"]["items"][0]["artists"]) + " - " + \
					f"[{res['tracks']['items'][0]['name']}]({res['tracks']['items'][0]['preview_url']})"
...
async def show_current_song(client, message):
	try:
		...
		text = f"**{join_artists(res['item']['artists'])}** - [{res['item']['name']}]({res['item']['preview_url']}) (_{res['item']['album']['name']}_) " + \
